...
def power_on_vm(vm_id, data, user_id=None):
    ...
    do_power_on_vm.delay(dummy_vm_id, user_id=user_id)


@app.task
def do_power_on_vm(dummy_vm_id, user_id=None):
    with transaction.atomic():
...
def power_off_vm(vm_id, data, user_id=None):
    ...
    with transaction.atomic():
        dummy_vm_id = VM.objects.get(id=vm_id).dummyvm.id
    do_power_off_vm.delay(dummy_vm_id, user_id=user_id)


@app.task
def do_power_off_vm(dummy_vm_id, user_id=None):
    with transaction.atomic():
        dvm = DummyVM.objects.get(id=dummy_vm_id)
        vm_id = dvm.vm.id
